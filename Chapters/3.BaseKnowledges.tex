	...
	\newpage
		...
		\begin{center}
			\begin{figure}[htp]
				...
				\end{center}
				\caption{ Phân tích thanh ghi \protect\footnotemark}
				\label{fig:Flow}
			\end{figure}
			\footnotetext{Trích dẫn hình ảnh trong sách Assembly Language}
		\end{center}
...
.
	...
	Cách mà thông tin được lưu trữ trong bộ nhớ phụ thuộc vào chế độ hoạt động của bộ xử lý (chế bộ bảo vệ hoặc chế độ địa chỉ thưc) và kích thước giá trị của toán hạng (32-bit hay 16-bit). Từ hình~\ref{fig:ProtectedFPU32} đến hình~\ref{fig:ProtectedFPU16}. Trong chế độ virtual-8086 hoặc SMM, địa chỉ thực thể hiện trong hình~\ref{fig:ProtectedFPU16}..
	\begin{center}
			\begin{figure}[htp]
				\begin{center}
...
\newpage
	...
	\begin{center}
		...
		\begin{longtable}{|l|l|p{1.5cm}|l|l|}
		 \hline
		...
		\hline	
		 ...
		 \hline		
		  Gói số nguyên & 80 & 18 (cơ số 10) &Không phù hợp & (-$10^{18}$ + 1) tới $(10^{18}$– 1)  \\
		 \hline			
